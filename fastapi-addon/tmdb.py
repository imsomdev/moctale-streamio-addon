...
def _cache_key(title: str, year: str, content_type: str, api_key: str) -> str:
    return f"{api_key[:8]}|{title.lower().strip()}|{year}|{content_type}"
...
async def get_tmdb_meta(
    ...
    key = _cache_key(title, year, expected_type, api_key)
    if key in _cache:
        return _cache[key]
